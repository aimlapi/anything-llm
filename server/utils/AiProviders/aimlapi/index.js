...
class AimlapiLLM {
  ...
  constructor(embedder = null, modelPreference = null) {
    if (!process.env.AIMLAPI_LLM_API_KEY)
      throw new Error("No AI/ML API key was set.");

    this.className = "AimlapiLLM";
    const { OpenAI: OpenAIApi } = require("openai");
    this.basePath = "https://api.aimlapi.com/v1";
  ...
  /**
   * AI/ML API has various models that never return `finish_reasons` and thus leave the stream open
   * which causes issues in subsequent messages. This timeout value forces us to close the stream after
   * x milliseconds. This is a configurable value via the AIMLAPI_LLM_TIMEOUT_MS value
   * @returns {number} The timeout value in milliseconds (default: 3_000)
   */
  #parseTimeout() {
    this.log(
      `AI/ML API timeout is set to ${process.env.AIMLAPI_LLM_TIMEOUT_MS ?? this.defaultTimeout}ms`
    );
    if (isNaN(Number(process.env.AIMLAPI_LLM_TIMEOUT_MS)))
      return this.defaultTimeout;
    const setValue = Number(process.env.AIMLAPI_LLM_TIMEOUT_MS);
  ...
  // The AI/ML API model API has a lot of models, so we cache this locally in the directory
  ...
  async #syncModels() {
    ...
    this.log(
      "Model cache is not present or stale. Fetching from the AI/ML API API."
    );
  ...
  handleStream(response, stream, responseProps) {
    ...
    return new Promise(async (resolve) => {
      ...
      // NOTICE: Not all AI/ML API models will return a stop reason
      ...
      // no new chunks then we kill the stream and assume it to be complete. AI/ML API is quite fast
      ...
      const timeoutCheck = setInterval(() => {
        ...
        if (diffMs >= timeoutThresholdMs) {
          this.log(
            `AI/ML API stream did not self-close and has been stale for >${timeoutThresholdMs}ms. Closing response stream.`
          );
...
/**
 * Fetches available models from AI/ML API and keeps only chat-completion models.
 * The AI/ML API model list can list the same model id more than once (one entry
 * per endpoint it supports, e.g. `openai/chat-completions` vs
 * `openai/responses/submit`) — filter to the chat-completions endpoint and
 * dedupe by id.
 */
async function fetchAimlapiModels() {
  return await fetch(`https://api.aimlapi.com/v1/models`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${process.env.AIMLAPI_LLM_API_KEY}`,
    },
  })
    .then((res) => res.json())
    .then(({ data = [] }) => {
      ...
      const chatModels = data.filter(
        (model) => model.type === "openai/chat-completions"
      );

      chatModels.forEach((model) => {
        if (models.hasOwnProperty(model.id)) return;
        models[model.id] = {
          id: model.id,
          name: model.info?.name || model.id,
          organization: model.info?.developer || model.id.split("/")[0],
          maxLength: model.info?.contextLength || 4096, // Conservative default
        };
      });
    ...
    .catch((e) => {
      console.error("Error fetching AI/ML API models:", e);
      return {};
    });
}
